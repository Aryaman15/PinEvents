...
export const configureSockets = (server: HttpServer, allowedOrigins: string[]) => {
  ...
  io.on("connection", (socket) => {
    socket.on(
      "join",
      async (eventId: string, callback?: (response: { ok?: boolean; error?: string }) => void) => {
        try {
          if (!eventId) {
          ...
          const event = await Event.findById(eventId).select("_id acceptedMembers").lean();
          if (!event) {
            callback?.({ error: "Event not found" });
            return;
          }
          const accepted = await isAcceptedMember(eventId, userId, event.acceptedMembers);
          if (!accepted) {
