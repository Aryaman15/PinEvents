...
const savedEventsKey = 'savedEvents';
...
export default function App() {
  ...
  const [savedEvents, setSavedEvents] = useState<EventPin[]>([]);
  ...
  const eventFeatures = useMemo(() => {
    ...
    return {
      ...
      features: events.map((event) => {
        ...
        const isExpired = new Date(event.endTime).getTime() <= now;
        return {
          type: 'Feature',
          id: event.id,
          properties: {
  ...
  useEffect(() => {
    const loadToken = async () => {
      ...
      setIsLoading(false);
      const storedSavedEvents = await SecureStore.getItemAsync(savedEventsKey);
      if (storedSavedEvents) {
        try {
          setSavedEvents(JSON.parse(storedSavedEvents) as EventPin[]);
        } catch (error) {
          setSavedEvents([]);
        }
      }
  ...
  const persistSavedEvents = async (nextEvents: EventPin[]) => {
    setSavedEvents(nextEvents);
    await SecureStore.setItemAsync(savedEventsKey, JSON.stringify(nextEvents));
  };

  const scheduleEventReminder = async (eventToSave: EventPin) => {
    const startTime = new Date(eventToSave.startTime).getTime();
    ...
    await Notifications.scheduleNotificationAsync({
      content: {
        ...
        body: `${eventToSave.title} starts in 1 hour.`,
      },
      trigger: new Date(reminderTime),
    });
  };

  const handleSaveEvent = async (eventToSave: EventPin) => {
    if (savedEvents.some((event) => event.id === eventToSave.id)) {
      return;
    }
    const nextEvents = [...savedEvents, eventToSave];
    await persistSavedEvents(nextEvents);
    await scheduleEventReminder(eventToSave);
  };
  ...
  const handleJoinEvent = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/events/${selectedEvent.id}/join`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${authToken}`,
      ...
      if (!response.ok) {
        setErrorMessage('Unable to join event.');
        return;
      }

      const data = (await response.json()) as { attendeeCount?: number };
      setSelectedEventDetail((prev) =>
        prev ? { ...prev, attendeeCount: data.attendeeCount ?? prev.attendeeCount } : prev
      );
      setEvents((prev) =>
        prev.map((event) =>
          event.id === selectedEvent.id
            ? { ...event, attendeeCount: data.attendeeCount ?? event.attendeeCount }
            : event
        )
      );
      await handleSaveEvent(selectedEvent);
    } catch (error) {
      setErrorMessage('Unable to reach the server.');
    }
  };
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={[styles.profileContainer, styles.safeAreaContent]}>
          ...
          <Text style={styles.subtitle}>Your attended and signed-up events.</Text>
          {savedEvents.length ? (
            savedEvents.map((event) => {
              const isCompleted = new Date(event.endTime).getTime() <= Date.now();
              return (
                <View key={event.id} style={styles.savedEventCard}>
                  ...
                  <Text style={styles.savedEventMeta}>
                    {formatDateTime(event.startTime)} • {event.category}
                  </Text>
                  <Text style={isCompleted ? styles.completedStatus : styles.upcomingStatus}>
            ...
            <Text style={styles.mutedText}>No saved events yet.</Text>
          )}
          <Pressable style={styles.linkButton} onPress={() => setShowSavedEvents(false)}>
            <Text style={styles.linkText}>Back to map</Text>
          </Pressable>
          <StatusBar style="dark" />
        </ScrollView>
  ...
  const isEventSaved = selectedEvent
    ? savedEvents.some((event) => event.id === selectedEvent.id)
    : false;
  const canJoinPublicEvent = selectedEvent?.type === 'public' && !isEventSaved;
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable style={styles.savedEventsButton} onPress={() => setShowSavedEvents(true)}>
        <Text style={styles.savedEventsButtonText}>Saved</Text>
      </Pressable>
      ...
      {selectedEventId ? (
        <View style={styles.bottomSheet}>
          ...
          {canJoinPublicEvent ? (
            <Pressable style={styles.primaryButton} onPress={handleJoinEvent}>
