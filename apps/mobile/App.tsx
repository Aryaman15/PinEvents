...
export default function App() {
  ...
  const [eventDetail, setEventDetail] = useState<EventPin | null>(null);
  ...
  const loadEventDetail = async (token: string, eventId: string) => {
    if (!apiUrl) {
      setErrorMessage('EXPO_PUBLIC_API_URL is not set.');
      return;
    }
    ...
    try {
      ...
      const data = (await response.json()) as { event?: EventPin };
      if (!data.event) {
        setErrorMessage('Event response is missing.');
        return;
      }
  ...
  const loadJoinRequests = async (token: string, eventId: string) => {
    if (!apiUrl) {
      setErrorMessage('EXPO_PUBLIC_API_URL is not set.');
      return;
    }
    ...
    try {
      ...
      const data = (await response.json()) as { requests?: EventRequest[] };
  ...
  const handleRequestJoin = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/events/${selectedEventId}/request-join`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      ...
      setErrorMessage('Request sent. Waiting for approval.');
    } catch (error) {
      setErrorMessage('Unable to reach the server.');
    }
  };
  ...
  const handleApproveRequest = async (requestId: string) => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/events/${selectedEventId}/requests/${requestId}/approve`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${authToken}`,
      ...
      await loadJoinRequests(authToken, selectedEventId);
    } catch (error) {
      setErrorMessage('Unable to reach the server.');
    }
  };
  ...
  const handleRejectRequest = async (requestId: string) => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/events/${selectedEventId}/requests/${requestId}/reject`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${authToken}`,
  ...
  const handleEventPress = (event: MapLibreGL.OnPressEvent) => {
    ...
    setRequests([]);
    if (authToken) {
      loadEventDetail(authToken, eventId).catch(() => {
        // Errors handled in loadEventDetail.
      });
    }
  };
          ...
          {eventDetail?.role === 'admin' ? (
            <View style={styles.requestsContainer}>
              ...
              <Pressable
                ...
                onPress={() => {
                  if (authToken && selectedEventId) {
                    loadJoinRequests(authToken, selectedEventId).catch(() => {
              ...
              >
                <Text style={styles.secondaryButtonText}>
                  {isLoadingRequests ? 'Loading...' : 'Refresh requests'}
                </Text>
              </Pressable>
              {requests.length === 0 ? (
                ...
                requests.map((request) => (
                  <View key={request.id} style={styles.requestRow}>
                    <View style={styles.requestInfo}>
                      <Text style={styles.requestLabel}>User:</Text>
                      <Text style={styles.requestValue}>{request.userId}</Text>
                    </View>
